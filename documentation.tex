...
%% Envoyer toutes remarques a Nicolas.Delestre@insa-rouen.fr
		...
		\item Modifications de puis la version 2.6
			\begin{itemize}
				\item Amélioration de la notation des pointeurs
			\end{itemize}

		\item Modifications depuis la version 2.4
			\begin{itemize}
				\item Ajoute des commandes \textbackslash typeProcedure, \textbackslash typeFonction, \textbackslash typeProcedureAvecPreconditions, \textbackslash typeFonctionAvecPreconditions
			\end{itemize}

		\item Modifications depuis la version 2.3.8
			\begin{itemize}
				\item Ajout de la commande \textbackslash tadOperationAvecPreconditions
				\item Ajout des environnement tadPreconditions et des commandes \textbackslash motclefTADPreconditions		et \textbackslash tadPrecondition
				\item Ajout de la commande \textbackslash champ
			\end{itemize}


		\item Modifications depuis la version 2.3.7
			\begin{itemize}
				\item Ajout des types de base naturelNonNul, reelPositif, reelPositifNonNul (resp Negatif)
				\item Elimination d'accents dans les mots cles lies à l'algorithmique
				\item Ajout de la commande \textbackslash commentaire
			\end{itemize}
				
		\item Modification depuis la version 2.3.6
			\begin{itemize}
				\item Ajout des commandes \textbackslash fonctionAvecPreconditions \textbackslash signatureFonctionAvecPreconditions, \textbackslash procedureAvecPreconditions et \textbackslash signatureProcedureAvecPreconditions
				\item Remplacement de structure et champEnregistrement par enregistrement et champEnregistrement
			\end{itemize}
			
		\item Modification depuis la version 2.3.5
			\begin{itemize}
				\item suppression de la commande \textbackslash programme
			\end{itemize}
		
		\item Modification depuis la version 2.3.4
			\begin{itemize}
				\item Dans les commandes fonction et procedure, modification du test pour savoir si les variables locales sont presentes ou pas
				\item Suppression de l'erreur de compilation du a la redefinition de l'environnement structure
			\end{itemize}

		\item Modification depuis la version 2.3.2
			\begin{itemize}
				\item structure est maintenant un environnement qui s'integre dans l'environnement algorithme
				\item \textbackslash champEnregistrement a pris une majuscule
				\item Apparition de \texttt{\textbackslash tadSemantiquesAxiomatiques}
				\item Apparition de l'environnement \texttt{\textbackslash tadAxiomes} et commande \texttt{\textbackslash tadAxiome}
				\item Apparition de \texttt{\textbackslash typePointeur}, \texttt{\textbackslash pointeur},\texttt{\textbackslash \pointeurNULL}, \texttt{\textbackslash champPointeur}, \texttt{\textbackslash allouer}, \texttt{\textbackslash desallouer}
				\item Elimination de l'erreur de compilation lié à la définition de la commande \texttt{\textbackslash structure}
			\end{itemize}
...
\begin{lstlisting}
		...
		Naturel Non Nul : \naturelNonNul
		Réel: \reel
		Réel positif : \reelPositif
		Réel positif non nul : \reelPositifNonNul
		Réel négatif : \reelNegatif
		Réel négatif non nul : \reelNegatifNonNul
		Booléen: \booleen
		Caractère: \caractere
...
\begin{lstlisting}
		...
		{}
		{n: \naturel; nnn : \naturelNonNul; e : \entier; r \reel;rp : \reelPositif; rpnn : \reelPositifNonNul;rn : \reelNegatif; rnnn : \reelNegatifNonNul; b: \booleen; c: \caractere; ch: \chaine}
		{}
		{}
		{}
		{\instruction{corps du programme}}
...
\subsection{Les types fonctions et procedures}
Les commandes pour les types de fonction et procédures sont les suivantes :
\begin{lstlisting}
type fonction : \typeFonction{1}{2}
(1): Les paramètres
(2): Le type de retour

type fonction avec preconditions : \typeFonctionAvecPreconditions{1}{2}{3}
(1): Les paramètres
(2): Le type de retour
(3): Les préconditions

type procédure : \typeProcedure{1}
(1): Les paramètres

type procédure : \typeProcedureAvecPreconditions{1}{2}
(1): Les paramètres
(2): Les préconditions
\end{lstlisting}
...
\begin{lstlisting}
	...
	{}
	{
		\typeFonction{\reel , \reel}{\booleen},\\
		\typeFonctionAvecPreconditions{a : \reel}{\reel}{a div 2 = 0},\\
		\typeProcedure{\paramEntreeSortie{a : \reel}},\\
		\typeProcedureAvecPreconditions{\paramEntreeSortie{a : \reel}}{a div 2 = 0},\\
	}
	{}
	{}
	{}
...
Pour insérer une fonction sans précondition, il faut utiliser la commande \texttt{\textbackslash fonction}		qui prend en paramètre cinq arguments :
\begin{lstlisting}
\fonction {1}{2}{3}{4}{5}
...
\begin{algorithme}
		...
		Pour insérer une fonction avec précondition, il faut utiliser la commande \texttt{\textbackslash fonctionAvecPreconditions}		qui prend en paramètre six arguments :
		\begin{lstlisting}
		\fonctionAvecPrecondition {1}{2}{3}{4}{5}{6}
			(1) Le nom de la fonction 
			(2) Les paramètres 
			(3) Le type de retour
			(4) Les variables locales
...
\paragraph{Sans précondition}
Pour insérer une signature de fonction sans précondition, il faut utiliser la commande
...
\signaturefonction {1}{2}{3}
 ...
 \paragraph{Avec précondition}
 Pour insérer une signature de fonction avec précondition, il faut utiliser la commande
 \texttt{\textbackslash signatureFonctionAvecPreconditions} qui prend en paramètre quatre
 arguments identiques aux quatre premiers paramètres de la commande \texttt{\textbackslash fonction}
\begin{lstlisting}
 \signatureFonctionAvecPrecondition {1}{2}{3}{4}
  (1) Le nom de la fonction 
  (2) Les paramètres 
  (3) Le type de retour
  (4) Les préconditions
...
\paragraph{Sans précondition}
Pour insérer une procédure sans précondition, il faut utiliser la commande\texttt{\textbackslash procedure} qui prend en paramètre quatre arguments:
\begin{lstlisting}
\procedure {1}{2}{3}{4} 
...
\paragraph{Avec précondition}
Pour insérer une procédure avec précondition, il faut utiliser la commande\texttt{\textbackslash procedureAvecPreconditions} qui prend en paramètre cinq arguments:
\begin{lstlisting}
\procedureAvecPrecondition {1}{2}{3}{4}{5} 
	(1) nom de la procédure
	(2) Les paramètres
	(3) Les variables locales
	(4) Les préconditions
...
signature de procédure sans précondition à l'aide de la commande \texttt{\textbackslash
...
Tout comme pour les fonctions avec précondition, on a la possibilité d'écrire une
signature de procédure avec précondition à l'aide de la commande \texttt{\textbackslash
		signatureProcedureAvecPreconditions} qui prend en paramètre deux arguments identiques
aux trois premiers arguments de la commande \texttt{\textbackslash		procedureAvecPreconditions}

\begin{lstlisting}
\signatureProcedureAvecPreconditions{1}{2}{3}
 (1) nom de la procédure
 (2) Les paramètres
 (3) Les préconditions
\end{lstlisting}
...
\begin{lstlisting}
	\begin{tad}
		...
		\begin{tadOperations}{Dictionnaire}
			...
			\tadOperation{estPresent}{\tadUnParam{Dictionnaire}}{\tadUnParam{\booleen}}
			\tadOperationAvecPreconditions{retirer}{\tadDeuxParams{Dictionnire}{Clef}}{\tadUnParam{Element}}
		\end{tadOperations}
		\begin{tadPreconditions}{Dictionnaire}
			\tadPrecondition{retirer}{estPresent(dictionnaire,mot)}
...
\item 
 ...
 \item 
\begin{lstlisting}
	\begin{tadPreconditions}{1}
		\tadPrecondition{2}{3}
	\end{tadPreconditions}
\end{lstlisting}
L'environnement \texttt{tadPreconditions} permet de lister les éventuelles préconditions de la commande \texttt{\textbackslash tadPrecondition}
 ...
 (3) : La précondition de l'opération
...
\begin{lstlisting}
	\begin{tad}
		...
		\end{tadOperations}
		\begin{tadPreconditions}{Dictionnaire}
			\tadPrecondition{retirer}{estPresent(dictionnaire,clef)}
		\end{tadPreconditions}
		\begin{tadSemantiques}{Dictionnaire}
...
\subsection{La commande commentaire}
La commande \texttt{\textbackslash commentaire} sert à mettre ( donc ) des commentaires dans l'algorihme.
...
\begin{lstlisting}
	\begin{algorithme}
		...
		{\affecter{temp}{val1}
				...
				\affecter {val2}{temp}
				\remarque{Une procédure ne renvoie pas de valeur }
				\commentaire{Encore une fois procédure ne renvoie pas de valeur}
		}
	\end{algorithme}
\end{lstlisting}
